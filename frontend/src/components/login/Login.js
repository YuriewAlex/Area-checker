...
function Login() {
        ...
        fetch("http://localhost:8080/login" + body, {
            method: "POST"
                ...
                if (response.ok) {
                    ...
                    MessageInstance.show({severity: 'error', summary: 'Incorrect login or password'});
                }
            }
        ))
    }
    ...
    const signUp = e => {
        ...
        fetch("http://localhost:8080/register" + body, {
            ...
            if (response.ok) {
                MessageInstance.show({severity: 'success', summary: 'Registration complete'});
                console.log(json)
                console.log(json.login)
                //    store.dispatch({type: "change", value: json.login});
            } else {
                ...
                MessageInstance.show({severity: 'error', summary: 'Login already used'});
            }
        }))
    }
