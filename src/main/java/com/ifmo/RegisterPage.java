...
import com.ifmo.input.UserCredentials;
import com.ifmo.input.UserChecker;
import com.ifmo.dbInterface.UserRepository;
import com.ifmo.input.PointService;
import com.ifmo.input.UserService;
...
public class RegisterPage {
    ...
    public RegisterPage(UserChecker userChecker, UserService userService, PointService pointService, UserRepository userRepository) {
    ...
    public void initBinder(WebDataBinder webDataBinder){
        webDataBinder.addValidators(userChecker);
    }
    ...
    public ResponseEntity login(@ModelAttribute("registerform")UserCredentials credentials, BindingResult bindingResult){
        if(bindingResult.hasErrors() || !userService.findByLoginAndPassword(credentials.getLogin(), userService.Sec(credentials.getPassword()))){
            Map<String, Object> resp = new HashMap<>();
            resp.put("error", "Ошибка в авторизации");
            return ResponseEntity.status(HttpStatus.BAD_GATEWAY).body(resp);
        }
        if(userService.findByLoginAndPassword(credentials.getLogin(), userService.Sec(credentials.getPassword()))) {
            Map<String, Object> resp = new HashMap<>();
            resp.put("login", credentials.getLogin());
            resp.put("password", credentials.getPassword());
